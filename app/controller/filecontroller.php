...
public $max_file_size = 1048576; // 1 MB expressed in bytes
#$max_file_size = 104; // 1 MB expressed in bytes
...
public $upload_path = "/var/www/projetophp.secure-websites/uploaded_files";

// Define allowed filetypes to check against during validations
public $allowed_mime_types = ['csv'];
public $allowed_extensions = ['csv'];
...
public function file_permissions($file) {
	...
	$numeric_perms = fileperms($file);				//return numeric value
	// but we are used to seeing the octal value
	$octal_perms = sprintf('%o', $numeric_perms);	//redifine the numeric retrurned value
...
public function file_contains_php($file) {
	...
	$position = strpos($contents, '<?php');		//search for the opening <?php tag
